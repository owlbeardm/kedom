...
const raceToClass = [
    {
        id: "all", classes: ["warrior", "expert", "mage", "necromancer", "elementalist",
            "partial-warrior", "partial-expert", "partial-mage", "partial-necromancer", "partial-elementalist",
            "partial-priest", "partial-wise", "partial-accursed", "partial-duelist", "partial-empath", "partial-rune-guardian", "partial-beast",]
    },
        ...
        <table>
            ...
            <tbody>
                <tr v-for="index in 10" :key="index">
                    ...
                    <td>+{{ (picked1.partial && (!!picked1.minAb || !!picked2?.minAb)) ? Math.ceil(calcAb(picked1,
                        picked2)
                        * (index) / 10) : Math.floor(calcAb(picked1, picked2) * index / 10) }}</td>
                    <td>
                        <span v-if="index == 1">Race Focus, {{ picked1.foci }}{{ picked1.foci.length > 0 ? ', ' : ''
                            }}<span v-if="picked1.partial">{{ picked2.foci }}{{ picked2.foci.length > 0 ? ', ' : ''
                                }}</span></span>
